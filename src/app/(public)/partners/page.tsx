...
function partnerCountByType(list: Partner[], type: PartnerType) {
...
function PartnerTypeCard({
  ...
  const count = partnerCountByType(partners, type);
  return (
...
export default function PartnersPage() {
  const { data, loading } = useContent<
    Pick<ContentData, "site" | "partners" | "testimonials">
  >(["site", "partners", "testimonials"]);
          ...
          <Stagger className="mt-12 grid gap-5 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5">
            {partnerTypes.map((type) => (
              <StaggerItem key={type} className="h-full">
                <PartnerTypeCard type={type} partners={partners} />
              </StaggerItem>
            ))}
          </Stagger>
        </div>
